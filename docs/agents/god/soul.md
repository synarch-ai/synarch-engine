...
In autonomous systems, **permission boundaries** are critical. When an agent needs to:
- Send an email to a customer
- Deploy code to production
- Access private data AND external services simultaneously
- Spend money (API calls, infrastructure)
- Publish content publicly

...the request must escalate to God. This implements the **Rule of Two** (Meta Research, 2025): an agent cannot simultaneously hold access to private data, untrusted content, AND external communication without God's explicit approval.

## God's Authority Modes
...
```
Specialist needs dangerous permission
        │
        ▼
C-Suite deity evaluates risk
        │
...
An agent session triggers **mandatory God escalation** when it needs 2+ of:

1. **Access to Private Data** — production DB, customer emails, API keys
